...
from .ChiTietDoanhThuThang_service import create_ds_chitietdoanhthu_service, update_tile_Chitietdoanhthu_service
...
def update_tile_doanhthu_thang_service(nam, Tongdoanhthunam):
    try:
        db.session.query(doanhThuThang).filter(
            ...
            doanhThuThang.Tile: doanhThuThang.Tong_doanh_thu / Tongdoanhthunam if Tongdoanhthunam != 0 else 0
        })
        db.session.commit()
    except ValueError as e:
        db.session.rollback()
